...
def _ret(c,n):return 0.0 if len(c)<=n or c[-n-1]==0 else (c[-1]/c[-n-1]-1.0)*100.0
...
def _frame(symbol: str, interval: str, limit: int=96) -> Optional[Dict[str,float]]:
    ...
    c,h,l,v=d["c"],d["h"],d["l"],d["v"];e9=_ema(c,9);e21=_ema(c,21);e55=_ema(c,55);a=_atr(h,l,c,14) or 0.0;adx=_adx(h,l,c,14);vr=v[-1]/(sum(v[-20:])/20.0+1e-12)
...
def plan(symbol: str, kernel=None) -> Optional[Dict[str,Any]]:
    ...
    for f,w in ((f5,0.45),(f15,0.35),(f1h,0.20)):
        s=0.0;s+=1.0 if f["e9"]>f["e21"] else -1.0;s+=0.8 if f["e21"]>f["e55"] else -0.8;s+=0.8 if f["mom8"]>0 else -0.8;s+=0.6 if f["mom20"]>0 else -0.6
        if f["adx"]>25:s*=1.12
        elif f["adx"]<16:s*=0.82
        votes.append(s*w)
    raw=sum(votes);side="LONG" if raw>0 else "SHORT";strength=min(1.0,abs(raw)/3.0);adx5=min(1.0,max(0.0,(f5["adx"]-15)/25));vol5=min(1.0,max(0.0,(f5["vol"]-0.75)/1.25));trend_alignment=sum(1 for f in frames if (f["e9"]>f["e21"]>f["e55"])==(side=="LONG"))/3.0
    confidence=100.0*(0.50*strength+0.20*adx5+0.15*vol5+0.15*trend_alignment);atr_pct=f5["atr_pct"]
    if atr_pct<=0:return None
    base_risk=_f(os.getenv("LIVE_RISK","0.10"),0.10);risk=base_risk*(0.72+0.68*min(1.0,confidence/100.0));vol_scale=0.055/max(atr_pct,0.055);risk*=max(0.65,min(1.35,vol_scale));risk=max(0.03,min(_f(os.getenv("AGGRESSIVE_MAX_RISK","0.14"),0.14),risk))
    lev_min=int(_f(os.getenv("LEV_MIN",os.getenv("MIN_LEVERAGE","40")),40));lev_max=int(_f(os.getenv("MAX_LEVERAGE","75"),75));lev_min=max(40,lev_min);lev_max=max(lev_min,lev_max);lev=int(round(lev_min+(lev_max-lev_min)*min(1.0,confidence/100.0)**1.35))
    stop_mult=1.05+0.55*(1.0-confidence/100.0);tp_mult=1.80+1.40*(confidence/100.0);sl_pct=max(0.45,min(2.20,atr_pct*stop_mult));tp_pct=max(0.80,min(5.50,atr_pct*tp_mult));fee=_f(os.getenv("FEE_RATE","0.0004"),0.0004);slip=_f(os.getenv("SCANNER_SLIPPAGE_BPS","2.0"),2.0)/10000.0;expected=abs(raw)*atr_pct/100.0*2.8*max(0.5,confidence/100.0);roundtrip=fee*2+slip
    if expected<=roundtrip*1.35:return None
    spread_bps=0.0
    if kernel is not None:
        try:
            bid,ask,_=kernel.book(symbol);spread_bps=max(0.0,(ask-bid)/((ask+bid)/2.0)*10000.0)
            if spread_bps>_f(os.getenv("AGGRESSIVE_MAX_SPREAD_BPS","8"),8):return None
        except Exception:return None
    return {"symbol":symbol,"side":side,"score":50+raw*16,"confidence":confidence,"risk_pct":risk,"leverage":lev,"tp_pct":tp_pct,"sl_pct":sl_pct,"atr_pct":atr_pct,"adx":f5["adx"],"spread_bps":spread_bps,"expected_edge":expected-roundtrip,"trend_alignment":trend_alignment}
