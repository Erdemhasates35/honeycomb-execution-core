...
class LiveExecutionAdapter implements ExecutionAdapter {
  ...
  constructor(options: { apiKey: string; apiSecret: string; market: FuturesMarket; testnet?: boolean }) {
    ...
    this.engine = new BinanceFuturesEngineCatE({
      apiKey: options.apiKey,
      apiSecret: options.apiSecret,
      testnet: options.testnet ?? false,
      marketType: options.market,
    });
  }
  ...
  async place(intent: OrderIntent): Promise<Fill> {
    ...
    const result = await this.engine.executeOrder({
    ...
    const price = Number(result?.avgPrice ?? result?.price ?? intent.referencePrice ?? await this.markPrice(intent.symbol));
    const fill: Fill = {
      orderId: String(result?.orderId ?? `LIVE-${Date.now()}`),
      symbol: intent.symbol,
      side: intent.side,
      quantity: intent.quantity,
      price,
      fee: 0,
      settlementAsset: this.market === 'COIN_M' ? 'COIN' : 'USDT',
      timestamp: Date.now(),
      mode: this.mode,
      market: this.market,
    };
  ...
  async close(symbol: string, referencePrice?: number): Promise<Fill | null> {
    ...
    const side = Number(active.positionAmt) > 0 ? 'SHORT' : 'LONG';
    return this.place({ symbol, side, quantity: Math.abs(Number(active.positionAmt)), referencePrice, reduceOnly: true });
  }

  snapshot(): ExecutionSnapshot {
    return { mode: this.mode, market: this.market, balance: 0, positions: [], fills: [...this.fills] };
  }
...
export class ExecutionRouter implements ExecutionAdapter {
  ...
  constructor(config: RouterConfig = {}) {
    ...
    if (this.mode === 'TEST') {
      this.adapter = new TestExecutionAdapter({ market: this.market, initialBalance: config.initialBalance });
    } else if (this.mode === 'PAPER') {
      this.adapter = new PaperExecutionAdapter({ market: this.market, initialBalance: config.initialBalance, feeRate: config.paperFeeRate, slippageBps: config.paperSlippageBps });
    } else {
  ...
  get testAdapter(): TestExecutionAdapter | undefined {
    return this.adapter instanceof TestExecutionAdapter ? this.adapter : undefined;
  }
}
