...
def reconcile_startup()->None:
    ...
    for p in data if isinstance(data,list) else []:
        ...
        side="LONG" if amt>0 else "SHORT";lev=max(40,min(75,int(float(p.get("leverage") or 40))));mark=float(p.get("markPrice") or entry)
        ...
        _open_meta[s]={"side":side,"entry":entry,"qty":abs(amt),"lev":lev,"tp":tp,"sl":sl,"confidence":0,"regime":"RECONCILED","margin":0.0,"opened":time.time(),"mfe":0.0,"mae":0.0}
...
def scan_symbol(symbol:str)->None:
    ...
    if os.getenv("LIVE_ARMED","0")!="1":audit.append({"event":"paper","symbol":symbol,"tech":{k:tech[k] for k in tech if k not in ("details","exit_surface")}});return
    try:
...
def check_closed_positions()->None:
    for symbol,meta in list(_open_meta.items()):
        ...
        if real_amt>0:
            ...
            continue
        try:
            trades=kernel._http("GET",kernel.v["userTrades"],{"symbol":symbol,"limit":50},signed=True,weight=5);net=sum(float(t.get("realizedPnl") or 0) for t in trades if str(t.get("side") or "").upper() in ("BUY","SELL"));alpha_core.on_trade_closed(symbol,meta["side"],net,meta.get("confidence") or 0,meta.get("regime") or "")
        except Exception:alpha_core.on_trade_closed(symbol,meta["side"],0.0,meta.get("confidence") or 0,meta.get("regime") or "")
