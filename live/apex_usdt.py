...
# -*- coding: utf-8 -*-
"""APEX USDT-M live engine — fill ledger, exchange protect, stale halt, single-flight."""
...
RISK = float(ENV.get("LIVE_RISK", "0.12"))
LEV = min(50, max(1, int(float(ENV.get("MAX_LEVERAGE", "20")))))
HOLD_MAX = int(ENV.get("HOLD_MAX", "18"))
INTERVAL = int(ENV.get("AUTO_INTERVAL_SEC", "5"))
MAX_POS = float(ENV.get("MAX_POSITION_SIZE_USDT", "200"))
...
def loop():
    ...
    log("START APEX-USDT bal=%.4f lev=%dx risk=%.0f%% margin=%s" % (k.balance(), LEV, RISK * 100, k.margin_type))
            ...
            if opens:
                ...
                if reason:
                    try:
                        ...
                        net = fill["rp"] if abs(fill["rp"]) > 1e-12 else (
                            ((fill["avg"] - pos["entry"]) if pos["side"] == "LONG" else (pos["entry"] - fill["avg"])) * pos["qty"] - fill["commission"])
                        with lock:
                            journal.insert(0, {"symbol": pos["symbol"], "side": pos["side"], "reason": reason,
                                "entry": pos["entry"], "exit": fill["avg"], "net_pnl": round(net, 6), "rp": round(fill["rp"], 6)})
                            positions.pop(pos["id"], None)
                        log("CLOSED %s %s %s NET=%.6f RP=%.6f" % (pos["side"], pos["symbol"], reason, net, fill["rp"]))
                ...
                if idle > 0: idle -= 1
                    ...
                    if side:
                        try:
                            ...
                            if notional < 5: log("SKIP low notional")
                            else:
                                opened = k.open_market(sym, side, notional, LEV, tp_pct, sl_pct)
                                pid = "a%d" % int(time.time() * 1000)
                                with lock:
                                    positions[pid] = {"id": pid, "status": "OPEN", "ticks": 0, "mfe": 0.0, **opened}
                                log("OPENED %s %s" % (sym, detail))
                        except Exception as e: log("OPEN skip %s: %s" % (sym, e))
                    elif i % 10 == 0: log("scan %s: %s" % (sym, detail))
            i += 1
...
def run_flask():
    ...
    app = Flask(__name__)
    @app.route("/health")
    def health(): return jsonify({"ok": 1, "engine": "APEX-USDT", "i": state["i"]})
    @app.route("/status")
    def status():
        with lock:
