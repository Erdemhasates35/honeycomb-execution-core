...
# -*- coding: utf-8 -*-
"""HELIX COIN-M — dapi only. Signs against https://dapi.binance.com."""
...
def loop():
    ...
    log("START HELIX-COIN bal=%.6f lev=%dx (dapi only)" % (k.balance(), LEV))
                ...
                if idle > 0: idle -= 1
                    ...
                    if side:
                        try:
                            ...
                            bal = k.balance()
                            notional = min(max(bal * RISK * LEV * px * 0.01, 10), MAX_POS)
...
def run_flask():
    ...
    app = Flask(__name__)
    @app.route("/health")
    def health(): return jsonify({"ok": 1, "engine": "HELIX-COIN", "i": state["i"]})
    @app.route("/status")
    def status():
        with lock:
