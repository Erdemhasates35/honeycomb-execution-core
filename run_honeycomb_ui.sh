...
if [ -f runtime/ui_control_plane.pid ] && kill -0 "$(cat runtime/ui_control_plane.pid)" 2>/dev/null; then
  kill "$(cat runtime/ui_control_plane.pid)" 2>/dev/null || true
fi
nohup python -m orchestrator.ui_control_plane > logs/ui_control_plane.log 2>&1 & echo $! > runtime/ui_control_plane.pid

for i in $(seq 1 20); do
  curl -fsS "http://127.0.0.1:${UI_CONTROL_PORT}/api/ui/capabilities" >/dev/null 2>&1 && break
  sleep 1
done

printf '\nHONEYCOMB UI GATEWAY: http://127.0.0.1:%s\n' "$UI_CONTROL_PORT"
curl -fsS "http://127.0.0.1:${UI_CONTROL_PORT}/api/ui/capabilities"
printf '\n'
curl -fsS "http://127.0.0.1:${UI_CONTROL_PORT}/api/ui/status"
printf '\n'
