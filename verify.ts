...
const apiKey = process.env.BINANCE_API_KEY || process.env.BINANCE_TESTNET_API_KEY || process.env.API_KEY || '';
const apiSecret = process.env.BINANCE_API_SECRET || process.env.BINANCE_SECRET || process.env.BINANCE_TESTNET_SECRET || process.env.API_SECRET || '';

interface SymbolMetrics { symbol: string; volume24h: number; volatility: number; momentumScore: number; atr: number; compositeScore: number; }
interface ActiveTrade { symbol: string; entryPrice: number; quantity: number; stopLossPrice: number; takeProfitPrice: number; }
...
class QuantumInstitutionalOrchestrator {
  ...
  private maxRiskPerTradeRatio: number = 0.015;
  private candidateUniverse: string[] = ['BTCUSDT','ETHUSDT','SOLUSDT','BNBUSDT','XRPUSDT','ADAUSDT','AVAXUSDT','DOGEUSDT','LINKUSDT','NEARUSDT'];

  constructor(engine: BinanceFuturesEngineCatE) { this.engine = engine; }
  ...
  public async rankAndSelectTopSymbols(): Promise<SymbolMetrics[]> {
    ...
    for (const symbol of this.candidateUniverse) {
      try {
        ...
        for (const k of klines) {
          ...
          highs.push(high); lows.push(low); closes.push(close); totalVolume += volume * close;
        }
        const currentPrice = closes[closes.length - 1], price24hAgo = closes[0];
        const momentumScore = (currentPrice - price24hAgo) / price24hAgo;
        let atrSum = 0;
        for (let i = 1; i < closes.length; i++) atrSum += Math.max(highs[i]-lows[i], Math.abs(highs[i]-closes[i-1]), Math.abs(lows[i]-closes[i-1]));
        ...
        metricsList.push({symbol, volume24h:totalVolume, volatility, momentumScore, atr, compositeScore});
      } catch (err) { console.warn(`[QUANTUM WARN]: ${symbol} verisi işlenemedi.`); }
    }
    ...
    top10.forEach((m,idx) => console.log(`  #${idx+1} ${m.symbol} | Skor: ${m.compositeScore.toFixed(4)} | Hacim: $${Math.round(m.volume24h).toLocaleString()} | ATR: ${m.atr.toFixed(4)}`));
    return top10;
  }
  ...
  public async executeAndManagePortfolio(topSymbols: SymbolMetrics[], totalUsdtBalance: number): Promise<void> {
    ...
    for (const metric of topSymbols.slice(0, this.maxConcurrentTrades)) {
      ...
      try { await this.engine.setMarginType(symbol, 'ISOLATED'); await this.engine.setLeverage(symbol, 10); } catch (e) { /* mevcut ayarı koru */ }
      ...
      try {
        const raw = await this.engine.executeOrder(order);
        const res = raw as Partial<{orderId:string}>;
        console.log(`[EXECUTION SUCCESS]: ${symbol} İletildi | OrderId: ${res.orderId || 'OK'}`);
        this.activeTrades.set(symbol,{symbol,entryPrice:markPrice,quantity:parseFloat(quantity),stopLossPrice,takeProfitPrice});
      } catch (err) { console.error(`[EXECUTION ERROR]: ${symbol} Hatası:`, (err as Error).message); }
    }
  }
}
...
async function runVerification() {
  ...
  if (!apiKey || !apiSecret) { console.error('[CRITICAL ERROR]: API Key veya API Secret `.env` dosyasında bulunamadı!'); process.exit(1); }
  console.log('[VERIFY]: API Anahtarları `.env` Üzerinden Doğrulandı.');
  const liveArmed = process.env.EXECUTION_MODE?.toUpperCase() === 'LIVE' && process.env.LIVE_ARMED === '1';
  if (!liveArmed) console.log('[VERIFY]: LIVE emir yolu KAPALI — yalnızca analiz/doğrulama yapılacak.');

  const engine = new BinanceFuturesEngineCatE({apiKey, apiSecret, testnet:false});
  ...
  try {
    ...
    if (availableBalance > 10 && liveArmed) await orchestrator.executeAndManagePortfolio(top10, availableBalance);
    else if (!liveArmed) console.log('[VERIFY NOTICE]: LIVE_ARMED/EXECUTION_MODE kapalı; emir gönderilmedi.');
    else console.log('[VERIFY NOTICE]: Bakiye sınırlı olduğundan analiz modunda tamamlandı.');
    console.log('[STATUS]: doğrulama tamamlandı.');
  } catch (err) { console.error('[RUNTIME EXCEPTION]:', (err as Error).message); }
  finally { setTimeout(() => { engine.terminate(); process.exit(0); }, 3000); }
}
